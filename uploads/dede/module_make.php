...
else if($action=='edit')
{
    ...
    // 不允许这些字符
    $readmetxt = preg_replace("#(/\*)[\s\S]*(\*/)#i", '', $readmetxt);
    ...
    foreach (explode(",", $cfg_disable_funs) as $value) {
        ...
        if(!empty($value) && preg_match("#[^a-z]+['\"]*{$value}['\"]*[\s]*[([{']#i", " {$readmetxt}") == TRUE) {
            $readmetxt = dede_htmlspecialchars($readmetxt);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$readmetxt}</pre>");
        }
    }
    
    if(preg_match("#^[\s\S]+<\?(php|=)?[\s]+#i", " {$readmetxt}") == TRUE) {
        if(preg_match("#[$][_0-9a-z]+[\s]*[(][\s\S]*[)][\s]*[;]#iU", " {$readmetxt}") == TRUE) {
            $readmetxt = dede_htmlspecialchars($readmetxt);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$readmetxt}</pre>");
        }
        if(preg_match("#[@][$][_0-9a-z]+[\s]*[(][\s\S]*[)]#iU", " {$readmetxt}") == TRUE) {
            $readmetxt = dede_htmlspecialchars($readmetxt);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$readmetxt}</pre>");
        }
        if(preg_match("#[`][\s\S]*[`]#i", " {$readmetxt}") == TRUE) {
            $readmetxt = dede_htmlspecialchars($readmetxt);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$readmetxt}</pre>");
        }
    }
