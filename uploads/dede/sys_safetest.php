...
{
    global $message, $info, $cfg_basedir;
    $str = '';
    ...
    foreach (explode("|", $info) as $value) {
        ...
        if(!empty($value) && preg_match("#[^a-z]+['\"]*{$value}['\"]*[\s]*[([{']#i", " {$str}") == TRUE) {
            $trfile = preg_replace("#^".$cfg_basedir."#", '', $f);
            $filename = pathinfo($trfile, PATHINFO_BASENAME);
            $activepath = pathinfo($trfile, PATHINFO_DIRNAME);
            ...
            <div style='float:left'>[<a href='file_manage_view.php?fmdo=del&filename=$filename&activepath=$activepath' target='_blank'><u>删除</u></a>]
            [<a href='file_manage_view.php?fmdo=edit&filename=$filename&activepath=$activepath' target='_blank'><u>查看源码</u></a>]
            </div></div>\r\n";
            return 1;
        }
    }
