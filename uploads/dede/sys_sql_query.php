...
if($dopost=="viewinfo")
{
    ...
    {
        $dsql->SetQuery("SHOW CREATE TABLE `{$tablename}`");
        $dsql->Execute('me');
...
else if($dopost=="query")
{
    ...
    if(preg_match("#general_log#i", $sqlquery))
    {
        echo "<span style='font-size:10pt'>语句 $sqlquery 不允许在这里执行。</span>";
        exit();
    }
    if(preg_match("#outfile#i", $sqlquery))
    {
        echo "<span style='font-size:10pt'>语句 $sqlquery 不允许在这里执行。</span>";
        exit();
    }
