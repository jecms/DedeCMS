...
class DedeTagParse
{
    ...
    {
        if(!empty($str) && preg_match("#(script)[\s]*(language)[\s]*(=)[\s]*['\"]{1}(php)#i", " {$str}") == TRUE) {
            $errmsg = dede_htmlspecialchars("DedeCMS Error:<script language= 'php'>...</script>");
            return FALSE;
        }
    ...
    function RunPHP(&$refObj, $i)
    {
        ...
        // 不允许这些字符
        $phpcode = preg_replace("#(/\*)[\s\S]*(\*/)#i", '', $phpcode);
