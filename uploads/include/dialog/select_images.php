...
while($file = $dh->read()) {
   ...
   <a href=# onclick=\"ReturnImg('$reurl');\" $lstyle>$file</a></td>
...
<?php $noeditor = !empty($noeditor)?"<input type='hidden' name='noeditor' value='yes'>":''; echo $noeditor;//（2011.08.25 根据用户反馈修正图片上传回调 by:DedeCMS团队）?>
...
<td background="img/tbg.gif" bgcolor="#99CC00">
  ...
  <input type='checkbox' name='needwatermarkup' value='1' class='np' <?php if($photo_markup=='1') echo "checked"; ?> />水印
